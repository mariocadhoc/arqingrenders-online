document.addEventListener("DOMContentLoaded", () => {
  const includes = [
    { id: "header", url: "/components/header-es.html", event: "header-loaded" },
    { id: "footer", url: "/components/footer-es.html", event: "footer-loaded" }
  ];
...
document.addEventListener("header-loaded", () => {
  ...
  navLinks.forEach(link => {
    ...
    if (linkPath === "/es/" && currentPath === "/es/") {
      link.classList.add("active");
    } else if (linkPath !== "/es/" && currentPath.startsWith(linkPath)) {
      link.classList.add("active");
    }
  });
});
