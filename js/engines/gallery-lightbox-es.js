...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const UI_TEXT = {
    ...
    detailLabel: (label, index) => `${label} detalle ${index + 1}`
  };
  ...
  function buildThumbContainer(cardData, colIndex) {
    ...
    cardData.children.forEach((child, index) => {
      ...
      img.alt = child.alt || UI_TEXT.detailLabel(cardData.label, index);
      img.decoding = 'async';
  ...
  function updateZoomHintState() {
    ...
    DOM.zoomHintText.textContent = zoomState.scale > 1.02 ? UI_TEXT.clickToFit : UI_TEXT.clickToZoom;
  }
  ...
  function replaceMainImage(newSrc, altText = UI_TEXT.projectImage) {
  ...
  function initLightboxDOM() {
    ...
    lb.innerHTML = `
      ...
      <button class="gp-lightbox-close" type="button" aria-label="${UI_TEXT.closeLightbox}">✕</button>
      <div class="gp-lightbox-main-wrapper">
        <div class="gp-lightbox-main-viewport">
          <div class="gp-lightbox-main-media">
            <img class="gp-lightbox-main-img" src="" alt="${UI_TEXT.projectImage}" />
          </div>
          <div class="gp-lightbox-zoom-hint" hidden>
            ${getMagnifierIcon()}
            <span>${UI_TEXT.clickToZoom}</span>
          </div>
        </div>
      </div>
  ...
  function openLightbox(cardEl) {
    ...
    const mediaItems = [
      { thumbSrc: cardData.img, fullSrc: cardData.full || cardData.img, alt: UI_TEXT.imageLabel(cardData.label, 0) },
      ...(cardData.children || [])
    ...
    mediaItems.forEach((item, idx) => {
      ...
      btn.setAttribute('aria-label', UI_TEXT.openImage(idx));
      const img = document.createElement('img');
      img.src = item.thumbSrc || item.fullSrc;
      img.alt = item.alt || UI_TEXT.imageLabel(cardData.label, idx);
      img.decoding = 'async';
      img.loading = 'eager';
      btn.appendChild(img);
      btn.addEventListener('click', () => {
        ...
        syncActiveThumb(btn);
        replaceMainImage(item.fullSrc, item.alt || UI_TEXT.imageLabel(cardData.label, idx));
      });
      DOM.sidebar.appendChild(btn);
    });

    DOM.mainImg.src = mediaItems[0].fullSrc;
    DOM.mainImg.alt = mediaItems[0].alt || UI_TEXT.imageLabel(cardData.label, 0);
