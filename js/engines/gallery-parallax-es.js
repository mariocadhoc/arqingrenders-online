  ...
  function gp_normalizeCard(entry, index, parentCardScale) {
    ...
    if (entry.type === 'title' || entry.isTitle) {
      return {
        id: index,
        isTitle: true,
        text: gp_pickText(entry.text, entry.label, GP_UI_LABELS.selectedProjects),
        category: '_Title',
    ...
    return {
      ...
      category: gp_pickText(entry.category, GP_UI_LABELS.uncategorized),
  ...
  function gp_buildFilterButtons(cardsData) {
    ...
    categories.forEach((category) => {
      ...
      btn.textContent = category === 'All'
        ? GP_UI_LABELS.all
        : (category === 'Archive' ? GP_UI_LABELS.archive : category);
